...
export default observer(
  ({
    ...
    useEffect(() => {
      ...
      // 数据
      areaChart.current.data(dataList)
      // 度量
      areaChart.current.scale({
        // 纵轴
        y: {
          nice: true,
          min: 0,
        ...
        x: xTicks
          ? { type: 'cat', ticks: xTicks, range: [0, 1] }
          : { type: 'cat', nice: true, range: [0, 1] },
      })

      // 设置横轴
      areaChart.current.axis('x', {
        // 取消刻度标
        tickLine: null,
        // 刻度值
        label: {
      ...
      // 伪装y轴单位
      yUnit &&
        areaChart.current.annotation().text({
          content: yUnit,
          position: ['start', 'end'],
          top: true,
          style: {
      ...
      areaChart.current.legend(
        showLegend && {
          ...
          marker: (name) => {
            ...
            // const avgItem = avgList?.find((el) => el?.label === name)
            return {
              symbol: legendOption?.markerSymbol || 'hyphen',
              spacing: legendOption?.markerSpacing || 14,
              style: groupKey
                ? { r: legendOption?.markerR || 13, lineWidth: legendOption?.markerLineWidth || 2 }
                : groupItem
                ? { r: legendOption?.markerR || 13, lineWidth: legendOption?.markerLineWidth || 2 }
                : {
                    r: legendOption?.markerR || 13,
                    lineWidth: legendOption?.markerLineWidth || 2,
      ...
      areaChart.current.tooltip({
        ...
        // 自定义title，格式化
        title: (title) => (xUnit ? xSuffix + title + xUnit : xSuffix + title),
        // 自定义模板的对应样式
        domStyles: {
          // 容器
          'g2-tooltip': {
      ...
      // 渲染
      areaChart.current.render()
      // 清理并销毁，还原
      return () => {
        areaChart.current?.clear(true)
    ...
    // 创建图表更新
    useEffect(() => {
      areaChart.current?.changeData(dataList)
    }, [dataList])
